...
<div
	class="fixed inset-0 z-[1100] flex items-start justify-center pt-[14vh] bg-[rgba(8,40,59,0.5)] backdrop-blur-[8px] transition-all duration-200 {open
...
>
	...
	<div
		class="w-[560px] max-w-[92vw] bg-surface-card border border-line rounded-xl overflow-hidden shadow-[var(--shadow-2xl)] transition-transform duration-[220ms] {open
			? 'translate-y-0'
			: 'translate-y-2'}"
		onclick={(e) => e.stopPropagation()}
	>
		<div class="flex items-center gap-3 px-[18px] py-4 border-b border-line">
			<svg
				width="18"
				height="18"
				viewBox="0 0 24 24"
			...
			<input
				...
				onkeydown={handleKey}
				oninput={() => (cmdActive = 0)}
				class="flex-1 border-0 outline-none bg-transparent text-base text-ink [font-family:var(--font-body)] placeholder:text-ink-muted/60"
			/>
			<kbd
				class="[font-family:var(--font-mono)] text-[11px] px-2 py-[3px] border border-line rounded text-ink-muted"
				>esc</kbd
			>
		</div>
		...
		<div class="max-h-[56vh] overflow-auto p-2">
			{#if filteredCmdItems.length === 0}
				<div class="px-[14px] py-5 text-ink-muted text-[13px]">
					No matches. Try "work", "stack", or "github".
				</div>
			{/if}
			{#each filteredCmdItems as item, i}
				<div
					class="flex items-center gap-[14px] px-[14px] py-3 rounded-md cursor-pointer text-ink text-sm transition-colors duration-100 {i === cmdActive
						? 'bg-surface-alt'
				...
				>
					...
					{item.label}
					<span class="ml-auto [font-family:var(--font-mono)] text-[11px] text-ink-muted"
						>{item.meta}</span
					>
				</div>
			{/each}
		...
		<div
			class="px-[14px] py-[10px] border-t border-line [font-family:var(--font-mono)] text-[11px] text-ink-muted flex gap-4 justify-between"
		>
